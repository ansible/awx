...
def popen_wrapper(args, os_err_exc_type=Exception, stdout_encoding='utf-8'):
    ...
    try:
        ...
        strerror = force_str(e.strerror, DEFAULT_LOCALE_ENCODING, strings_only=True)
        raise Exception(os_err_exc_type, os_err_exc_type('Error executing %s: %s' % (args[0], strerror)), sys.exc_info()[2])
    output, errors = p.communicate()
    return (
        force_str(output, stdout_encoding, strings_only=True, errors='strict'),
        force_str(errors, DEFAULT_LOCALE_ENCODING, strings_only=True, errors='replace'),
        p.returncode,
    )
