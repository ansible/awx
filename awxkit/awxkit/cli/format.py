...
def add_authentication_arguments(parser, env):
    ...
    auth.add_argument(
        '--conf.host',
        default=env.get('CONTROLLER_HOST', 'https://127.0.0.1:443'),
        metavar='https://example.awx.org',
    )
    auth.add_argument(
        '--conf.token',
        default=env.get('CONTROLLER_OAUTH_TOKEN', env.get('CONTROLLER_TOKEN', '')),
        help='an OAuth2.0 token (get one by using `awx login`)',
        metavar='TEXT',
    )
    auth.add_argument(
        '--conf.username',
        default=env.get('CONTROLLER_USERNAME', 'admin'),
        metavar='TEXT',
    )
    auth.add_argument(
        '--conf.password',
        default=env.get('CONTROLLER_PASSWORD', 'password'),
        metavar='TEXT',
    )
    auth.add_argument(
        '-k',
        ...
        default=not strtobool(env.get('CONTROLLER_VERIFY_SSL', 'True')),
        action='store_true',
    )
...
def add_output_formatting_arguments(parser, env):
    ...
    formatting.add_argument(
        ...
        default=env.get('CONTROLLER_FORMAT', 'json'),
    ...
    formatting.add_argument(
        ...
        default=env.get('CONTROLLER_COLOR', 't'),
        type=strtobool,
    )
    formatting.add_argument(
        '-v',
        ...
        default=strtobool(env.get('CONTROLLER_VERBOSE', 'f')),
        action="store_true",
    )
