...
import InventoryHelpText from '../Inventory.helptext';

export const SourceVarsField = ({ popoverContent }) => {
  const helpText = InventoryHelpText();
...
export const OptionsField = ({ showProjectUpdate = false }) => {
  ...
  const helpText = InventoryHelpText(null, projectField);
        ...
        <FormGroup fieldId="option-checkboxes" label={t`Update options`}>
          <FormCheckboxLayout>
            ...
            <CheckboxField
              ...
              tooltip={helpText.subFormOptions.updateOnLaunch}
            />
            {showProjectUpdate && (
              <CheckboxField
