...
class UnifiedJobSerializer(BaseSerializer):
    # user_capabilities displayed are same for all sub models
...
class UnifiedJobListSerializer(UnifiedJobSerializer):
    ...
    _capabilities_prefetch = [
        {'start': [
            'unified_job_template.jobtemplate.execute_role',
            'unified_job_template.inventorysource.inventory.update_role',
            'unified_job_template.project.update_role',
            'unified_job_template.workflowjobtemplate.organization.execute_role'
        ]},
        {'delete': [
            'unified_job_template.jobtemplate.project.organization.admin_role',
            'unified_job_template.jobtemplate.inventory.organization.admin_role',
            'unified_job_template.workflowjobtemplate.organization.admin_role'
        ]}
    ]
    ...
    def to_representation(self, obj):
        ...
        if serializer_class:
            ...
            if self.parent:
                ...
                serializer.polymorphic_base = self
                # capabilities prefetch is only valid for these models
                if isinstance(obj, (Job, WorkflowJob, ProjectUpdate, InventoryUpdate)):
                    serializer.capabilities_prefetch = self._capabilities_prefetch
                else:
                    serializer.capabilities_prefetch = None
            ret = serializer.to_representation(obj)
    ...
    def get_related(self, obj):
        ...
        if obj.source_project_update_id:
            res['source_project_update'] = self.reverse('api:project_update_detail',
                                                        kwargs={'pk': obj.source_project_update_id})
        return res
