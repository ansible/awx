...
class InstanceInstanceLinksList(ParentMixin, ListAPIView):

    name = _("Instance's Instance Links")
    parent_model = models.Instance
    ...
    def get_queryset(self):
        po = self.get_parent_object()
        qs = get_user_queryset(self.request.user, models.InstanceLink)
        source = qs.filter(source=po.id)
        target = qs.filter(target=po.id)
        return source.union(target)


class InstancePeersList(ParentMixin, ListAPIView):

    name = _("Instance's Peers")
    parent_model = models.Instance
    model = models.Instance
    serializer_class = serializers.InstanceSerializer
    parent_access = 'read'
