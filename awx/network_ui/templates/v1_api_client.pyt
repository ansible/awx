...
def list_{{model.name.lower()}}(**kwargs):
    response = requests.get(util.get_url() + '{{model.end_point}}',  verify=util.get_verify(), auth=util.get_auth(), params=kwargs)
    return response.json()

def get_{{model.name.lower()}}({%for field in model.fields%}{%if field.pk%}{{field.name}}{%endif%}{%endfor%}):
    response = requests.get(util.get_url() + "{{model.end_point}}" + str({%for field in model.fields%}{%if field.pk%}{{field.name}}{%endif%}{%endfor%}), verify=util.get_verify(), auth=util.get_auth())
    return response.json()
...
def create_{{model.name.lower()}}({%for field in model.fields%}{%if not field.pk and field.default is not defined%}{{field.name}},{%endif%}{%endfor%}{%for field in model.fields%}{%if not field.pk and field.default is defined%}{{field.name}}={{field.default}},{%endif%}{%endfor%}):
    ...
    response = requests.post(util.get_url() + "{{model.end_point}}", data=json.dumps(dict({%for field in model.fields%}{%if not field.pk%}{{field.name}}={{field.name}},
                                                                                   {%endif%}{%endfor%})),
...
def update_{{model.name.lower()}}({%for field in model.fields%}{%if field.pk%}{{field.name}}{%endif%}{%endfor%}, {%for field in model.fields%}{%if not field.pk%}{{field.name}}=None,{%endif%}{%endfor%}):
    ...
    response = requests.patch(util.get_url() + "{{model.end_point}}" + str({%for field in model.fields%}{%if field.pk%}{{field.name}}{%endif%}{%endfor%}) + "/",
...
def delete_{{model.name.lower()}}({%for field in model.fields%}{%if field.pk%}{{field.name}}{%endif%}{%endfor%}):
    response = requests.delete(util.get_url() + "{{model.end_point}}" + str({%for field in model.fields%}{%if field.pk%}{{field.name}}{%endif%}{%endfor%}), verify=util.get_verify(), auth=util.get_auth())
    return response
