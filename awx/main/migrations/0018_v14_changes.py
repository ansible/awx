...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for host in orm.Host.objects.filter(active=True):
            ...
            if host.has_active_failures != has_active_failures:
                host.has_active_failures = has_active_failures
                host.save()
        
        # Update has_active_failures and hosts_with_active_failures for all
        # groups.
        def get_all_hosts_for_group(group, except_group_pks=None):
            except_group_pks = except_group_pks or set()
            except_group_pks.add(group.pk)
            qs = group.hosts.distinct()
        ...
        for group in orm.Group.objects.filter(active=True):
            ...
            changed = False
            if group.hosts_with_active_failures != hosts_with_active_failures:
                group.hosts_with_active_failures = hosts_with_active_failures
                changed = True
            if group.has_active_failures != has_active_failures:
                group.has_active_failures = has_active_failures
                changed = True
            if changed:
                group.save()
        ...
        for inventory in orm.Inventory.objects.filter(active=True):
            ...
            has_active_failures = bool(hosts_with_active_failures)
            changed = False
            if inventory.hosts_with_active_failures != hosts_with_active_failures:
                inventory.hosts_with_active_failures = hosts_with_active_failures
                changed = True
            if inventory.has_active_failures != has_active_failures:
                inventory.has_active_failures = has_active_failures
                changed = True
            if changed:
                inventory.save()
