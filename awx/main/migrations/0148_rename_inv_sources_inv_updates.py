...
def forwards(apps, schema_editor):
    ...
    if tower_type is not None and not CredentialType.objects.filter(managed_by_tower=True, namespace='controller').exists():
        logger.info('Renaming the Ansible Tower credential type for existing install')
        tower_type.namespace = 'controller'
        tower_type.save(update_fields=['namespace'])
...
def backwards(apps, schema_editor):
    ...
    CredentialType = apps.get_model('main', 'CredentialType')
    tower_type = CredentialType.objects.filter(managed_by_tower=True, namespace='controller').first()
