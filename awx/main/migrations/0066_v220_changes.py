...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for credential in orm.Credential.objects.order_by('-pk'):
            if credential.sudo_username:
                credential.become_method = 'sudo'
                credential.become_username = credential.sudo_username
                credential.become_password = decrypt_field(credential, 'sudo_password')
                credential.become_password = encrypt_field(credential, 'become_password', True)
                credential.save()
            elif credential.su_username:
                credential.become_method = 'su'
                credential.become_username = credential.su_username
                credential.become_password = decrypt_field(credential, 'su_password')
                credential.become_password = encrypt_field(credential, 'become_password', True)
                credential.save()
