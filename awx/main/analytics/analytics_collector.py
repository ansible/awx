...
import insights_analytics_collector
...
class AnalyticsCollector(insights_analytics_collector.Collector):
    def __init__(self,
                 collection_type=insights_analytics_collector.Collector.SCHEDULED_COLLECTION,
                 collector_module=collectors):
        super(AnalyticsCollector, self).__init__(collection_type=collection_type,
        ...
        if self.is_shipping_enabled():
            if not settings.INSIGHTS_TRACKING_STATE:
                logger.log(self.log_level,
                           "Insights for Ansible Automation Platform not enabled. "
                           "Use --dry-run to gather locally without sending.")
                return False

            if not (settings.AUTOMATION_ANALYTICS_URL and settings.REDHAT_USERNAME and settings.REDHAT_PASSWORD):
                logger.log(self.log_level,
                           "Not gathering analytics, configuration is invalid. "
                           "Use --dry-run to gather locally without sending.")
                return False
    ...
    # def pg_advisory_lock(self, key, blocking=False):
    #     with advisory_lock(key, wait=blocking) as acquired:
    #         yield acquired
    ...
    def registered_collectors(cls, module=None):
        ...
        return super(AnalyticsCollector).registered_collectors(collectors)
    ...
    def _load_last_gathered_entries(self):
        ...
        return json.loads((last_entries.value if last_entries is not None else '') or '{}',
    ...
    def _save_last_gather(self):
        ...
        with disable_activity_stream():
            if not settings.AUTOMATION_ANALYTICS_LAST_GATHER or \
                    self.gather_until > settings.AUTOMATION_ANALYTICS_LAST_GATHER:
