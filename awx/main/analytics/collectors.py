...
def _identify_lower(key, since, until):
    from awx.conf.models import Setting
...
def _events_slicing(key, since, until, query_func):
    lower, last_entries = _identify_lower(key, since, until)
...
def events_slicing_partitioned_modified(key, since, until):
    ...
    return _events_slicing(key, since, until, query_func)


def events_slicing_unpartitioned(key, since, until):
    def query_func(lower, until):
