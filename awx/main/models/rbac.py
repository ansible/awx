...
class Role(models.Model):
    ...
    role_field = models.TextField(null=False, help_text=_('Type of permission granted by this role.'))
    singleton_name = models.TextField(
        null=True, default=None, db_index=True, unique=True, help_text=_('role_field but only for system level roles with no associated content_object.')
    )
    parents = models.ManyToManyField('Role', related_name='children', help_text=_('Roles that implicitly grant this role.'))
    implicit_parents = models.TextField(null=False, default='[]', help_text=_('Cached id list of parent roles.'))
    ...
    content_type = models.ForeignKey(ContentType, null=True, default=None, on_delete=models.CASCADE, help_text=_('Resource type that this role applies for.'))
    object_id = models.PositiveIntegerField(null=True, default=None, help_text=_('The id of the resource this role applies to.'))
