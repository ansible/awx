...
class CredentialType(CommonModelNameNotUnique):
    ...
    native_source = models.CharField(
        max_length=1024,
        null=True,
        default=None,
        editable=False
    ...
    def from_db(cls, db, field_names, values):
        ...
        if instance.managed_by_tower and instance.native_source:
            native = NativeCredentialType.registry[instance.native_source]
    ...
    def defaults(cls):
        return dict(
            ...
            for k, v in NativeCredentialType.registry.items()
        )

    @classmethod
    def setup_tower_managed_defaults(cls):
        for default in NativeCredentialType.registry.values():
            existing = CredentialType.objects.filter(name=default.name, kind=default.kind).first()
            if existing is not None:
                existing.native_source = default.namespace
                existing.inputs = {}
                existing.injectors = {}
                existing.save()
                continue
...
class NativeCredentialType(SimpleNamespace):
    ...
    def __init__(self, namespace, **kwargs):
        for k in ('inputs', 'injectors'):
            if k not in kwargs:
                kwargs[k] = {}
        super(NativeCredentialType, self).__init__(namespace=namespace, **kwargs)
        if namespace in NativeCredentialType.registry:
            raise ValueError(
                'a NativeCredentialType with namespace={} is already defined'.format(namespace)
            )
        NativeCredentialType.registry[namespace] = self

    def create(self):
        return CredentialType(
            native_source=self.namespace,
            kind=self.kind,
            name=self.name,
