...
def build_safe_env(env):
    ...
    for k, v in safe_env.items():
        if k == "AWS_ACCESS_KEY_ID":
            ...
            k.startswith("ANSIBLE_")
            and not k.startswith("ANSIBLE_NET")
            and not k.startswith("ANSIBLE_GALAXY_SERVER")
        ):
            continue
...
class Credential(PasswordFieldsModel, CommonModelNameNotUnique, ResourceMixin):
    ...
    class Meta:
        ...
        permissions = [
            ("use_credential", "Can use credential in a job or related resource")
        ]
    ...
    def needs_ssh_password(self):
        return (
            self.credential_type.kind == "ssh" and self.inputs.get("password") == "ASK"
        )
    ...
    def needs_ssh_key_unlock(self):
        if self.credential_type.kind == "ssh" and self.inputs.get("ssh_key_unlock") in (
            "ASK",
            "",
        ):
    ...
    def needs_become_password(self):
        return (
            self.credential_type.kind == "ssh"
            and self.inputs.get("become_password") == "ASK"
        )

    @property
    def needs_vault_password(self):
        return (
            self.credential_type.kind == "vault"
            and self.inputs.get("vault_password") == "ASK"
        )
    ...
    def get_input(self, field_name, **kwargs):
        ...
        """
        if (
            self.credential_type.kind != "external"
            and field_name in self.dynamic_input_fields
        ):
        ...
        if self.organization:
            if isinstance(actor, User):
                if actor.is_superuser:
                    return
                if (
                    Organization.access_qs(actor, "member")
            ...
            raise DRFValidationError(
                {
                    "detail": _(
                        f"You cannot grant credential access to a {actor._meta.object_name} not in the credentials' organization"
                    )
                }
            )
...
class CredentialType(CommonModelNameNotUnique):
    ...
    namespace = models.CharField(
        max_length=1024, null=True, default=None, editable=False
    )
    ...
    def get_absolute_url(self, request=None):
        return reverse(
            "api:credential_type_detail", kwargs={"pk": self.pk}, request=request
        )
    ...
    def defaults(cls):
        return dict(
            (k, functools.partial(CredentialTypeHelper.create, v))
            for k, v in ManagedCredentialType.registry.items()
        )

    @classmethod
    def _get_credential_type_class(
        cls, apps: Apps = None, app_config: AppConfig = None
    ):
        """
    ...
    @classmethod
    def _setup_tower_managed_defaults(
        cls, apps: Apps = None, app_config: AppConfig = None
    ):
        ...
        for default in ManagedCredentialType.registry.values():
            existing = ct_class.objects.filter(
    ...
    def setup_tower_managed_defaults(
        ...
        with (
            advisory_lock(
                cls.CREDENTIAL_REGISTRATION_ADVISORY_LOCK_NAME, wait=wait_for_lock
            )
            if lock
            else nullcontext(True) as acquired
        ):
            if acquired:
    ...
    def inject_credential(
        self, credential, env, safe_env, args, private_data_dir, container_root=None
    ):
    ...
    def get_input_value(self, context: dict | None = None):
        ...
        for field_name, value in self.source_credential.inputs.items():
            if field_name in self.source_credential.credential_type.secret_fields:
                backend_kwargs[field_name] = decrypt_field(
                    self.source_credential, field_name
                )
            else:
...
def _is_oidc_namespace_disabled(ns):
    ...
    return ns in OIDC_CREDENTIAL_TYPE_NAMESPACES and not getattr(
        settings, "FEATURE_OIDC_WORKLOAD_IDENTITY_ENABLED", False
    )
...
def load_credentials():
    ...
    awx_entry_points = {
        ep.name: ep for ep in entry_points(group="awx_plugins.managed_credentials")
    }
    supported_entry_points = {
        ep.name: ep
        for ep in entry_points(group="awx_plugins.managed_credentials.supported")
    }
    plugin_entry_points = (
        awx_entry_points
        if detect_server_product_name() == "AWX"
        else {**awx_entry_points, **supported_entry_points}
    )
        ...
        if ns in ManagedCredentialType.registry:
            raise ValueError(
                "a ManagedCredentialType with namespace={} was defined in {}, but also defined in {}".format(
                    ns,
                    ep.value,
                    inspect.getsourcefile(ManagedCredentialType.registry[ns].__class__),
                )
            )
        ManagedCredentialType.registry[ns] = cred_plugin

    credential_plugins = {
        ep.name: ep for ep in entry_points(group="awx_plugins.credentials")
    }
    if detect_server_product_name() == "AWX":
        credential_plugins = {}
