...
class WorkflowJobNode(WorkflowNodeBase):
    ...
    def build_ancestor_artifacts(self):
        '''
        build ancestor artifacts: combine artifacts from before last run with
        any artifacts generated by the last run
        '''
        aa_dict = {}
    ...
    def get_job_kwargs(self):
        ...
        '''
        self.build_ancestor_artifacts()  # update artifacts regardless of job type
        ujt_obj = self.unified_job_template
        if ujt_obj is not None:
            prompts = self.prompts_dict()

            # combine extra_vars from node with any vars from WFJT or artifacts
            extra_vars = prompts.get('extra_vars', {})  # node extra_data lowest precedence
            if self.ancestor_artifacts:
                functional_aa_dict = copy(self.ancestor_artifacts)
                functional_aa_dict.pop('_ansible_no_log', None)
                extra_vars.update(functional_aa_dict)  # ancestor artifacts next precedence
            if self.workflow_job and self.workflow_job.extra_vars:
                extra_vars.update(self.workflow_job.extra_vars_dict)  # workflow job highest precedence
            if extra_vars:
                prompts['extra_vars'] = extra_vars

            # Follow reject/accept prompting rules of related template
            # errors are non-blocking here, missing files handled in scheduler
            data, ignored_fields, errors = ujt_obj._accept_or_ignore_job_kwargs(**prompts)
            if errors:
        ...
        if self.ancestor_artifacts and '_ansible_no_log' in self.ancestor_artifacts:
            for key in self.ancestor_artifacts:
