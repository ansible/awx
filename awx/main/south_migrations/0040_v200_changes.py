...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Set role on existing job events.
        for je in orm.JobEvent.objects.filter(event='playbook_on_task_start', role=''):
            name = je.event_data.get('name', '')
            task = je.event_data.get('task', '')
            if name and task and name.endswith(task) and ' | ' in name:
                role = name.split(' | ')[0]
                je.role = role
                je.save()
                for je2 in je.children.filter(role=''):
                    je2.role = role
                    je2.save()
    ...
    def backwards(self, orm):
        ...
        # Remove role from job events.
        for je in orm.JobEvent.objects.exclude(role=''):
            je.role = ''
            je.save()
