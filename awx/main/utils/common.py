...
def get_redis_client(broker_url):
    ...
    Args:
        broker_url: Redis connection URL (e.g., 'unix:///var/run/redis/redis.sock')

    Returns:
        redis.Redis: A Redis client instance configured with retry logic

    Notes:
        - Uses exponential backoff with 3 retries on ConnectionError and TimeoutError
        - Also retries on BusyLoadingError (when Redis is loading data)
...
async def get_redis_client_async(broker_url):
    """
