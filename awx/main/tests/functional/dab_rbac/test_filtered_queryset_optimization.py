...
class TestHostManagerDeferral:
    ...
    def test_existing_deferrals_preserved(self):
        """Pre-existing deferrals (last_job extras/artifacts) must still be present."""
        qs = Host.objects.all()
        deferred = qs.query.deferred_loading[0]
        for field in ('last_job__extra_vars', 'last_job__artifacts'):
            assert field in deferred, f'{field} should still be deferred. Deferred fields: {deferred}'
