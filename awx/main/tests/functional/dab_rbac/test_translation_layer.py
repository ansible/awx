...
from ansible_base.rbac.models import RoleUserAssignment, RoleDefinition, DABContentType
    ...
    for field in obj._meta.get_fields():
        if isinstance(field, ImplicitRoleField):
            ...
            if 'Compat' not in rd.name:
                model_rds = RoleDefinition.objects.filter(content_type=DABContentType.objects.get_for_model(obj))
                rd_data = {}
                for rd in model_rds:
...
def test_role_naming(setup_managed_roles):
    qs = RoleDefinition.objects.filter(content_type=DABContentType.objects.get(model='jobtemplate'), name__endswith='dmin')
...
def test_action_role_naming(setup_managed_roles):
    qs = RoleDefinition.objects.filter(content_type=DABContentType.objects.get(model='jobtemplate'), name__endswith='ecute')
    assert qs.count() == 1  # sanity
    rd = qs.first()
...
def test_compat_role_naming(setup_managed_roles, job_template, rando, alice):
    ...
    qs = RoleDefinition.objects.filter(content_type=DABContentType.objects.get(model='jobtemplate'), name__endswith='ompat')
    assert qs.count() == 1  # sanity
    rd = qs.first()
