...
from awx.main.models import Job, AdHocCommand, Credential, CredentialType, InventoryUpdate, InventorySource
...
class TestJobPrepCredentials:
    ...
    def test_job_uses_m2m_credentials(self, post, admin, project, inventory, machine_credential, credential):
        ...
        with mock.patch('awx.main.models.unified_jobs.UnifiedJob.signal_start'):
            response = post(reverse('api:job_template_launch', kwargs={'pk': jt.pk}), {}, admin, expect=201)
...
class TestProjectUpdatePrepCredentials:
    ...
    def test_project_update_uses_fk_credential(self, patch, admin, project, scm_credential, execution_environment):
    ...
    def test_project_update_no_credential(self, patch, admin, project, execution_environment):
        patch(
            reverse('api:project_detail', kwargs={'pk': project.pk}),
            {'credential': None},
            admin,
    ...
    def test_project_update_galaxy_credentials(self, post, admin, project, organization, execution_environment):
        ...
        post(
            reverse('api:organization_galaxy_credentials_list', kwargs={'pk': organization.pk}),
...
class TestInventoryUpdatePrepCredentials:
    ...
    def test_inventory_update_uses_m2m_credentials(self, post, admin, inventory, credential):
        inv_src = InventorySource.objects.create(name='ec2-src', inventory=inventory, source='ec2')
        ...
        with mock.patch('awx.main.tasks.system.update_inventory_computed_fields.apply_async'):
            response = post(
