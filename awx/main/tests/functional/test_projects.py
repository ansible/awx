...
@pytest.mark.django_db
def test_project_list_ordering_with_duplicate_names(get, order_by, organization_factory, admin):
    ...
    projects = []
    for i in range(5):
        projects.append(Project.objects.create(name=f'proj{i}', organization=Organization.objects.create(name=f'org{i}')))
    project_ids = {}
    for x in range(3):
