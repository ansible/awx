...
class TestInventoryScript:
    def test_hostvars(self, inventory):
        ...
        assert inventory.get_script_data(hostvars=True)["_meta"]["hostvars"][
            "ahost"
        ] == {"foo": "bar"}

    def test_towervars(self, inventory):
        host = inventory.hosts.create(name="ahost")
        assert inventory.get_script_data(hostvars=True, towervars=True)["_meta"][
            "hostvars"
        ]["ahost"] == {
...
class TestActiveCount:
    ...
    def test_active_count_minus_tower(self, inventory):
        ...
        source = inventory.inventory_sources.create(
            name="tower-source", source="controller"
        )
    ...
    def test_active_count_minus_constructed(self, organization):
        ...
        constructed = Inventory.objects.create(
            name="constructed-inv", kind="constructed", organization=organization
        )
...
class TestInventorySourceInjectors:
    def test_injector_credential_kind(self, project, credential):
        inventory_source = InventorySource.objects.create(
            name="foo", source="scm", source_project=project
        )
        inventory_source.credentials.add(credential)
        assert inventory_source.get_cloud_credential() == credential  # for serializer
        assert (
            inventory_source.injector_credential_kind() is None
        )  # scm has no dedicated injector
    ...
    def test_all_cloud_sources_covered(self):
        ...
        assert set(discover_available_cloud_provider_plugin_names()) | set(
            ["constructed"]
        ) == set(InventorySource.injectors.keys())
