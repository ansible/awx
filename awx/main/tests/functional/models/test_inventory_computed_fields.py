...
class TestInventoryComputedFields(TransactionTestCase):
    ...
    def _make_inventory(self, strategy='dense', Ng=3, Nh=3, inventory=None, initial_compute=True):
        if inventory:
            inv = inventory
        else:
        ...
        for i in range(Ng):
            ...
            elif strategy == 'linear' and prior_group:
                group.children.add(prior_group)
            groups.add(group)
            prior_group = group
        for i in range(Nh):
    ...
    def test_group_decedents_map(self):
        ...
        with self.assertNumQueries(2):  # one for groups & hosts
    ...
    def test_group_update(self):
        ...
        top_group.children.create(name='another-group', inventory=inv)
        top_group.update_computed_fields()
