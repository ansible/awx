...
def test_configure_validation_bypass_observability_wires_dab(mocker):
    ...
    assert 'awx.api.serializers' in allow.call_args[0][0]
    deny.assert_called_once()
    assert 'awx.main.models' in deny.call_args[0][0]
...
@pytest.mark.django_db
def test_audit_bulk_update_instances_logs_registered_text_field(caplog):
    job = Job.objects.create(job_explanation='<script>x</script>')
    with caplog.at_level(logging.WARNING, logger=LOGGER):
        audit_bulk_update_instances([job], ['job_explanation'])
    assert 'ORM bypass (bulk_update)' in caplog.text
    assert 'job_explanation' in caplog.text


@pytest.mark.django_db
def test_audit_bulk_update_instances_skips_unregistered_workflow_job(caplog):
    wj = WorkflowJob.objects.create(name='wf-job', job_explanation='<script>x</script>')
    with caplog.at_level(logging.WARNING, logger=LOGGER):
        audit_bulk_update_instances([wj], ['job_explanation'])
    assert 'ORM bypass (bulk_update)' not in caplog.text


@pytest.mark.django_db
def test_audit_bulk_update_instances_no_op_when_fields_empty(caplog):
    job = Job.objects.create(job_explanation='<script>x</script>')
    with caplog.at_level(logging.WARNING, logger=LOGGER):
        audit_bulk_update_instances([job], [])
    assert 'ORM bypass (bulk_update)' not in caplog.text
...
def test_task_manager_audits_before_job_explanation_bulk_update():
    ...
    audit_marker = 'audit_bulk_update_instances(tasks_to_update_job_explanation'
    ...
    assert source.index(audit_marker) < source.index(bulk_marker)
...
def test_db_bulk_update_sorted_by_id_audits_before_orm_bulk_update():
    ...
    assert 'audit_bulk_update_instances(sorted_objects, fields)' in source
    assert source.index('audit_bulk_update_instances') < source.index('model.objects.bulk_update')
