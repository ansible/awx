...
def test_pre_post_run_hook_facts(mock_create_partition, private_data_dir, execution_environment):
    ...
    # Mock RunJob task
    with override_db_settings(ANSIBLE_FACT_CACHE_TIMEOUT=False):
        task = jobs.RunJob()
        task.instance = job
...
@mock.patch('awx.main.tasks.jobs.create_partition', return_value=True)
def test_pre_post_run_hook_facts_deleted_sliced(mock_create_partition, private_data_dir, execution_environment):
...
@mock.patch('awx.main.tasks.facts.bulk_update_sorted_by_id')
def test_invalid_host_facts(bulk_update_sorted_by_id, private_data_dir, execution_environment):
