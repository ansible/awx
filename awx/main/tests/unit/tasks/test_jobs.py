...
def test_pre_post_run_hook_facts(
    mock_create_partition, mock_facts_settings, private_data_dir, execution_environment
):
...
def test_invalid_host_facts(
    ...
    hosts = [
        ...
        Host(
            id=1,
            name="host1",
            ansible_facts={"a": 1, "b": 2, "unexpected_key": "bad"},
            ansible_facts_modified=now(),
            inventory=mock_inventory,
        ),
    ]
    ...
    with pytest.raises(pytest.fail.Exception):
        if failures:
            pytest.fail(
                f" {len(failures)} facts cleared failures : {','.join(failures)}"
            )
        ...
        (
            {
                ...
                "unified_job_template": UnifiedJobTemplate(
                    pk=6, id=6, name="Test Unified Job Template"
                ),
        ...
        (
            {
                ...
                "job_type": "run",
                "launch_type": "manual",
                "organization": Organization(id=1, name=""),
            },
            {
                AutomationControllerJobScope.CLAIM_JOB_ID: 100,
...
def test_retrieve_workload_identity_jwt_returns_jwt_from_client(mock_get_client):
    ...
    assert (
        call_kwargs["claims"][AutomationControllerJobScope.CLAIM_JOB_NAME] == "Test Job"
    )
...
def test_retrieve_workload_identity_jwt_passes_audience_and_scope(mock_get_client):
    ...
    scope = "custom_scope"
    with mock.patch(
        "awx.main.tasks.jobs.populate_claims_for_workload", return_value={"job_id": 1}
    ):
    ...
    mock_client.request_workload_jwt.assert_called_once_with(
        claims={"job_id": 1}, scope=scope, audience=audience
    )
...
@mock.patch("awx.main.utils.workload_identity.get_workload_identity_client")
def test_retrieve_workload_identity_jwt_raises_when_client_not_configured(
    mock_get_client,
):
    ...
    with pytest.raises(
        RuntimeError, match="Workload identity client is not configured"
    ):
        jobs.retrieve_workload_identity_jwt(
            unified_job, audience="test_audience", scope="test_scope"
        )
...
def test_populate_workload_identity_tokens_passes_get_instance_timeout_to_client(
    mock_flag_enabled, mock_retrieve_jwt, effective_timeout, expected_ttl
):
    ...
    input_src.source_credential.credential_type.inputs = {
        "fields": [{"id": "workload_identity_token", "internal": True}]
    }
    ...
    prep = SimpleNamespace(
        ...
        workload_tokens={},
        galaxy_credentials=[],
        _instance=task.instance,
    )

    with mock.patch.object(
        task, "get_instance_timeout", return_value=effective_timeout
    ):
