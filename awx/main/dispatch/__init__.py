...
class PubSub(object):
    ...
    def current_notifies(conn):
        ...
        # Use conn.info.encoding to get the database encoding
        db_encoding = conn.info.encoding

        for pgn in ns:
            n = psycopg.connection.Notify(
                pgn.relname.decode(db_encoding),
                pgn.extra.decode(db_encoding),
                pgn.be_pid,
            )
            yield n
