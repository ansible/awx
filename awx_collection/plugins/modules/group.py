...
def main():
    ...
    argument_spec = dict(
        ...
        preserve_existing_children=dict(
            type="bool", default=False, aliases=["preserve_existing_groups"]
        ),
    ...
    group_fields = {
        "name": new_name
        if new_name
        else (module.get_item_name(group) if group else name),
    ...
    for resource, relationship in (("hosts", "hosts"), ("groups", "children")):
        ...
        for sub_name in name_list:
            ...
            if sub_obj is None:
                module.fail_json(
                    msg="Could not find {0} with name {1}".format(resource, sub_name)
                )
            id_list.append(sub_obj["id"])
        # Preserve existing objects
        if (preserve_existing_hosts and relationship == "hosts") or (
            preserve_existing_children and relationship == "children"
        ):
            if group:
                preserve_existing_check = module.get_all_endpoint(
