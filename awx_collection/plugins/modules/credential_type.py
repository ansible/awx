...
def main():
    ...
    credential_type = module.get_one(
        "credential_types", name_or_id=name, check_exists=(state == "exists")
    )
    ...
    credential_type_params["name"] = (
        new_name
        if new_name
        else (module.get_item_name(credential_type) if credential_type else name)
    )
