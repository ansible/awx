...
def main():
    ...
    organization = module.get_one(
        "organizations", name_or_id=name, check_exists=(state == "exists")
    )
    ...
    if galaxy_credentials is not None:
        ...
        for item in galaxy_credentials:
            association_fields["galaxy_credentials"].append(
    ...
    org_fields = {
        "name": new_name
        if new_name
        else (module.get_item_name(organization) if organization else name),
    }
    if description is not None:
        org_fields["description"] = description
    if default_ee is not None:
        org_fields["default_environment"] = module.resolve_name_to_id(
            "execution_environments", default_ee
        )
