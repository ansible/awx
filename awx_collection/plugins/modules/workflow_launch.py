...
def main():
    ...
    workflow_job_template = module.get_one(
        "workflow_job_templates", name_or_id=name, data=lookup_data
    )
    ...
    for variable_name, prompt in check_vars_to_prompts.items():
        if variable_name in post_data and not workflow_job_template[prompt]:
            param_errors.append(
                "The field {0} was specified but the workflow job template does not allow for it to be overridden".format(
                    variable_name
                )
            )
    ...
    if module.params.get("extra_vars") and not (
        workflow_job_template["ask_variables_on_launch"]
        or workflow_job_template["survey_enabled"]
    ):
        param_errors.append(
            "The field extra_vars was specified but the workflow job template does not allow for it to be overridden"
        )
    if len(param_errors) > 0:
        module.fail_json(
            msg="Parameters specified which can not be passed into workflow job template, see errors for details",
            errors=param_errors,
        )

    # Launch the job
    result = module.post_endpoint(
        workflow_job_template["related"]["launch"], data=post_data
    )

    if result["status_code"] != 201:
        module.fail_json(
            msg="Failed to launch workflow, see response for details", response=result
        )
