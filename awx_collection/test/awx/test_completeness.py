...
def test_meta_runtime():
    ...
    with open('{}/{}'.format(base_dir, meta_filename), 'r') as f:
        meta_data_string = f.read()

    meta_data = yaml.load(meta_data_string, Loader=yaml.Loader)

    needs_grouping = []
    for file_name in glob.glob('{}/{}/*'.format(base_dir, module_dir)):
    ...
    if needs_to_be_removed:
        print(cause_error("The following items should be removed from the {} {}:\n    {}".format(meta_filename, group, '\n    '.join(needs_to_be_removed))))

    if needs_to_be_added:
        print(cause_error("The following items should be added to the {} {}:\n    {}".format(meta_filename, group, '\n    '.join(needs_to_be_added))))
