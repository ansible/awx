...
def getTowerheader(self, header_name, default):
    mock_headers = {
        "X-API-Product-Name": controller_name,
        "X-API-Product-Version": ping_version,
    }
...
def test_version_warning(collection_import, silence_warning):
    ControllerAPIModule = collection_import(
        "plugins.module_utils.controller_api"
    ).ControllerAPIModule
    cli_data = {"ANSIBLE_MODULE_ARGS": {}}
    testargs = ["module_file2.py", json.dumps(cli_data)]
    with mock.patch.object(sys, "argv", testargs):
        with mock.patch(
            "ansible.module_utils.urls.Request.open", new=mock_awx_ping_response
        ):
            my_module = ControllerAPIModule(argument_spec=dict())
            my_module._COLLECTION_VERSION = "2.0.0"
            my_module._COLLECTION_FQCN = (
                "awx" + ".awx"
            )  # avoid FQCN replacement by script
            my_module.get_endpoint("ping")
    silence_warning.assert_any_call(
        "You are running collection version {0} but connecting to {1} version {2}".format(
            my_module._COLLECTION_VERSION, awx_name, ping_version
        )
    )
...
def test_version_warning_strictness_controller(collection_import, silence_warning):
    ...
    with mock.patch.object(sys, "argv", testargs):
        with mock.patch(
            "ansible.module_utils.urls.Request.open", new=mock_controller_ping_response
        ):
            my_module = ControllerAPIModule(argument_spec=dict())
            my_module._COLLECTION_VERSION = "1.2.0"
    ...
    silence_warning.assert_called_once_with(
        "You are running collection version {0} but connecting to {1} version {2}".format(
            my_module._COLLECTION_VERSION, controller_name, ping_version
        )
    )
...
def test_type_warning(collection_import, silence_warning):
    ...
    silence_warning.assert_called_once_with(
        "You are using the {0} version of this collection but connecting to {1}".format(
            my_module._COLLECTION_TYPE, awx_name
        )
    )
...
def test_duplicate_config(collection_import, silence_warning):
    ...
    data = {
        "name": "zigzoom",
        "zig": "zoom",
        "controller_username": "bob",
        "controller_config_file": "my_config",
    }
...
def test_conflicting_name_and_id(run_module, admin_user):
    ...
    result = run_module(
        "team",
        {"name": "foo_team", "description": "fooin around", "organization": slug},
        admin_user,
    )
